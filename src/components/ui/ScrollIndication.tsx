...
export const ScrollIndication = ({
  ...
  const scrollOpacity = useTransform(scrollYProgress, [0, 0.2], [1, 0.01]);
  const opacity = useSpring(scrollOpacity);

  return (
    <motion.div
      variants={fadeInFromTop}
      className={`lg:h-md:block absolute hidden ${String(positionClassName)}`}
    >
      <motion.div
        style={{ opacity }}
        className={`${flexDirection} lg:h-md:flex items-center justify-center`}
      >
